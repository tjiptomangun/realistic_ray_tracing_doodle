...
\end{center} 
\begin{enumerate}
\item Semua jajaran genjang rektangular (persegi panjang) dibatasi oleh
...
\draw(0, 0) node[anchor=east]{A} --++ (0: 4) node [anchor=west]{B}
	...
	--++(90:5);
\draw(5, -2) node[anchor=west]{$AB$ dan $BC$ adalah garis yang dimaksud};
\draw(5, -3) node[anchor=west]{$ABC$ adalah sudut yang dimaksud};
\end{tikzpicture}
\end{center}
...
dengan jumlah dari persegi panjang - persegi panjang yang dibentuk kedua garis 
tadi ketika belum dipotong dengan potongan - potongan yang dibentuk sesudah 
dipotong.
...
dan $BD$, ditambah $A$ dan $DE$ dan akhir oleh $A$ dan $EC$.
...
$EH$. Dan $BH$ adalah persegi panjang yang dibentuk $A$ dan $BC$.
